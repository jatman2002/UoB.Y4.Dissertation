...
for i in range(1,6):
    bookings = pd.read_csv(f'{os.getcwd()}/src/SQL-DATA/MLP-State/Restaurant-{i}-bookings.csv')
    ...
    train_data.to_csv(f'{os.getcwd()}/src/SQL-DATA/MLP-State/Restaurant-{i}-train.csv', sep=',', encoding='utf-8', index=False)
    # val_data.to_csv(f'C:/git/UoB.Y4.Dissertation/src/SQL-DATA/Restaurant-{i}-val.csv', sep=',', encoding='utf-8', index=False)
    test_data.to_csv(f'{os.getcwd()}/src/SQL-DATA/MLP-State/Restaurant-{i}-test.csv', sep=',', encoding='utf-8', index=False)
