...
for i in range(1,6):
    ...
    file_path = f'{os.getcwd()}/src/SQL-DATA/MLP-State-Soft-Label'

    bookings = pd.read_csv(f'{file_path}/Restaurant-{i}-bookings.csv')
    ...
    train_data.to_csv(f'{file_path}/Restaurant-{i}-train.csv', sep=',', encoding='utf-8', index=False)
    # val_data.to_csv(f'C:/git/UoB.Y4.Dissertation/src/SQL-DATA/Restaurant-{i}-val.csv', sep=',', encoding='utf-8', index=False)
    test_data.to_csv(f'{file_path}/Restaurant-{i}-test.csv', sep=',', encoding='utf-8', index=False)
